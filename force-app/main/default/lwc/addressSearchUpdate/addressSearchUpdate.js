...
export default class AddressSearchUpdate extends LightningElement {
  @track address = "";
  @track postalCode = "";
  @track streetAddress = "";
  @track detailAddress = "";
  ...
  getRecordIdFromUrl() {
    ...
    const match = url.match(/\/s\/detail\/(.*?)\//);
  ...
  handleAddressSave() {
    const FormData = {
      ...
      BillingStreet: this.streetAddress + " " + this.detailAddress
    };
    ...
    if (this.recordId == null) {
      ...
      const match = url.match(/\/s\/detail\/(.*?)\//);
      console.log(match[1]);
      updateFields({ recordId: match[1], data: FormData })
        .then((data) => {
